...
class ControllerExtensionPaymentPaygine extends Controller {
	...
	public function index() {
        ...
        if (isset($this->request->post['paygine_status'])) {
            $data['paygine_status'] = $this->request->post['paygine_status'];
        } else {
            $data['paygine_status'] = $this->config->get('paygine_status');
        }

        if (isset($this->request->post['paygine_sort_order'])) {
            $data['paygine_sort_order'] = $this->request->post['paygine_sort_order'];
        } else {
            $data['paygine_sort_order'] = $this->config->get('paygine_sort_order');
        }
        ...
        if (isset($this->request->post['paygine_sector'])) {
            $data['paygine_sector'] = $this->request->post['paygine_sector'];
        } else {
            $data['paygine_sector'] = $this->config->get('paygine_sector');
        }

        if (isset($this->request->post['paygine_kkt'])) {
            $data['paygine_kkt'] = $this->request->post['paygine_kkt'];
        } else {
            $data['paygine_kkt'] = $this->config->get('paygine_kkt');
        }

        if (isset($this->request->post['paygine_tax'])) {
            $data['paygine_tax'] = $this->request->post['paygine_tax'];
        } else {
            $data['paygine_tax'] = $this->config->get('paygine_tax');
        }

        if (isset($this->request->post['paygine_password'])) {
            $data['paygine_password'] = $this->request->post['paygine_password'];
        } else {
            $data['paygine_password'] = $this->config->get('paygine_password');
        }

        $data['callback'] = HTTP_CATALOG . 'index.php?route=payment/paygine/callback';

        if (isset($this->request->post['paygine_test'])) {
            $data['paygine_test'] = $this->request->post['paygine_test'];
        } else {
            $data['paygine_test'] = $this->config->get('paygine_test');
        }
