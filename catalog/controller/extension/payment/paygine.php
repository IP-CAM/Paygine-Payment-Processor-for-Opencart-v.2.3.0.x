...
class ControllerExtensionPaymentPaygine extends Controller {
    public function index() {
        ...
        if ($redirect_url) {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/payment/paygine.tpl')) {
                return $this->load->view($this->config->get('config_template') . 'extension/payment/paygine.tpl',  array(
                    'button_confirm' => $this->language->get('button_confirm'),
                    'action' => $redirect_url
                ));
            } else {
                return $this->load->view('extension/payment/paygine.tpl',  array(
                    'button_confirm' => $this->language->get('button_confirm'),
                    'action' => $redirect_url
                ));
            }
        } else {
            return $this->load->view('extension/payment/paygine_error.tpl', array(
    ...
    private function registerOrder($order_info) {
        ...
        $signature = base64_encode(md5($this->config->get('paygine_sector') . intval($amount * 100) . $currency . $this->config->get('paygine_password')));

        $fiscalPositions='';
        $fiscalAmount = 0;
        $KKT = $this->config->get('paygine_kkt');
        if ($KKT==1){
            $TAX = (strlen($this->config->get('paygine_tax')) > 0) ?
                intval($this->config->get('paygine_tax')) : 7;
            if ($TAX > 0 && $TAX < 7){
        ...
        if (intval($paygine_order_id) == 0) {
            // var_dump($paygine_order_id);
            error_log($paygine_order_id);
            return false;
        } else {
            $signature = base64_encode(md5($this->config->get('paygine_sector') . $paygine_order_id . $this->config->get('paygine_password')));
            return "{$paygine_url}/webapi/Purchase?sector={$this->config->get('paygine_sector')}&id={$paygine_order_id}&signature={$signature}";
        }
    ...
    private function checkPaymentStatus() {
        ...
        $signature = base64_encode(md5($this->config->get('paygine_sector') . $paygine_order_id . $paygine_operation_id . $this->config->get('paygine_password')));
    ...
    private function orderWasPayed($response) {
        ...
        $signature = base64_encode(md5(implode('', $tmp_response) . $this->config->get('paygine_password')));
